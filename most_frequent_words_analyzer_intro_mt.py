...
# define the worker
def worker(intro, all_token):
    intro_sel = cleanhtml(intro)
    ...
    if len(id_sic)>=10:     # so if the number of sic codes present is greater than 10 then proceed in the analysis
        ...
        for element in range(len(id_sic)):
            ...
            if intro_sel:
                ...
                pool.apply_async(worker, (intro_sel, all_token,))
        ...
        for icolumn in range(len(idx)):   # loop over the dataframe columns
            ...
            id0 = np.where(Counted_words[idx[icolumn]] > 0)[0]   # always to have a [0] index
...
files= glob.glob(os.path.join(dir, 'SIC_*_values_test.npz'), recursive=True)  # * means that loads all the possible SIC CODES available
